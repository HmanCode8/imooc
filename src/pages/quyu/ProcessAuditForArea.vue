...
const renderAllSegments = async () => {
  try {
    ...
    for (let i = 0; i < createForm.segments.length; i++) {
      ...
      if (seg.coords && seg.coords.length >= 3) {
        await addPolygon(seg.coords, getSegmentColor(i), "area-audit-draw");
      }
    }
  } catch (err) {
    console.error("渲染区域失败:", err);
  }
};

const startAreaDraw = async () => {
  try {
    ElMessage.info("请在地图上单击绘制区域，双击结束");
    const geom = await startDraw("Polygon", {
      layerId: "area-audit-draw",
      clearBefore: false,
      fitView: true,
  ...
  baseSubmitCreate({
    onSubmit: async ({ companyName, contactName, contactPhone }) => {
      ...
      for (let i = 0; i < createForm.segments.length; i++) {
        ...
        if (!segmentName) {
          ElMessage.warning(`区域 ${i + 1}：请输入区域名称`);
          currentSegmentIndex.value = i;
          return;
        }
        const segmentCode = String(seg.segmentCode || "").trim();
        if (!segmentCode) {
          ElMessage.warning(`区域 ${i + 1}：区域编码生成失败`);
          currentSegmentIndex.value = i;
          return;
        }
        if (!seg.districtCode) {
          ElMessage.warning(`区域 ${i + 1}：请选择所属区`);
          currentSegmentIndex.value = i;
          return;
        }
        if (!seg.streetCode) {
          ElMessage.warning(`区域 ${i + 1}：请选择所属镇街`);
          currentSegmentIndex.value = i;
          return;
        }
        if (!seg.coords || seg.coords.length < 3) {
          ElMessage.warning(`区域 ${i + 1}：请先在地图上绘制区域`);
          currentSegmentIndex.value = i;
          return;
        }
      ...
      const row = {
        ...
        contactName,
        contactPhone,
        totalAreaSqKm: Number(totalAreaSqKm.toFixed(2)),
...
onUnmounted(() => {
  globalStore.setAreaAuditDetailVisible(false);
  globalStore.setSelectedAreaAuditItem(null);
  globalStore.setSelectedAreaAuditSegmentId("");
  closeCreate();
});
</script>
      ...
      <div class="flex-1 overflow-y-auto p-4">
        <el-form label-width="86px">
          ...
          <div class="flex items-center justify-between mt-4 mb-2">
            <div class="text-sm font-bold text-gray-700">区域信息</div>

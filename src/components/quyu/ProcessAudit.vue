...
const getDistricts = async () => {
  try {
    const res = await userApi.getDistricts();
    console.log("getDistricts", res.data.districts);
...
const districtOptions = computed(() => {
  const districts = regionData.value.districts || [];
...
const startSegmentDraw = async () => {
  try {
    const layerId = `${props.auditType}-audit-draw`;
    ElMessage.info(
      `请在地图上单击${props.drawType === "Point" ? "添加点位" : "绘制" + props.title}，双击结束`,
    );
  ...
  baseSubmitCreate({
    onSubmit: async ({ companyName, contactName, contactPhone }) => {
      ...
      for (let i = 0; i < createForm.segments.length; i++) {
        ...
        if (!segmentName) {
          ElMessage.warning(
            `${segmentLabel.value} ${i + 1}：请输入${segmentLabel.value}名称`,
          );
        ...
        if (!segmentCode) {
          ElMessage.warning(
            `${segmentLabel.value} ${i + 1}：${segmentLabel.value}编码生成失败`,
          );
          currentSegmentIndex.value = i;
          return;
        }
        ...
        const isValidCoords =
          props.drawType === "Point"
            ? seg.coords?.length === 2
            : props.drawType === "LineString"
              ? seg.coords?.length >= 2
              : seg.coords?.length >= 3;

        if (!seg.coords || !isValidCoords) {
          ElMessage.warning(
            `${segmentLabel.value} ${i + 1}：请先在地图上绘制${props.title}`,
          );
          currentSegmentIndex.value = i;
          return;
        }
        ...
        const segmentObj = {
          ...
          type: props.drawType,
          coords: JSON.parse(JSON.stringify(seg.coords)),
        };
        // Add specific metric property if needed (areaSqKm, lengthKm, etc.)
        if (props.auditType === "line" || props.auditType === "resLine")
          segmentObj.lengthKm = metric;
  ...
  <div class="w-full h-full flex flex-col bg-white shadow-sm border-r border-gray-100 overflow-hidden">
    <div class="px-4 py-3 border-b border-gray-50 theme-bg">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <el-icon v-if="viewMode === 'create'" class="cursor-pointer hover:scale-110 transition-transform"
            @click="closeCreate">
            <ArrowLeft />
          </el-icon>
    ...
    <template v-if="viewMode === 'list'">
          ...
          <div class="flex items-center gap-6">
            <div class="flex-1 min-w-0">
              <el-form-item label="申请时间" class="w-full mb-0">
                <el-date-picker v-model="state.filters.dateRange" type="daterange" range-separator="至"
                  start-placeholder="开始时间" end-placeholder="结束时间" value-format="YYYY-MM-DD" class="w-full" />
              </el-form-item>
            </div>
            <div class="flex-1 min-w-0">
      ...
      <div class="flex-1 overflow-hidden">
        <el-table :data="pageRows" height="100%" v-loading="state.loading" @row-click="openDetail" stripe
          class="w-full">
      ...
      <div class="p-3 flex justify-end border-t border-gray-50 bg-white">
        <el-pagination v-model:current-page="state.page" v-model:page-size="state.pageSize" :total="total" background
          :page-sizes="[6, 10, 20, 50]" layout="total, sizes, prev, pager, next" />
      </div>
    </template>
      ...
      <div class="flex-1 overflow-y-auto p-4">
        <el-form label-width="86px">
          ...
          <div class="mb-2 flex items-center gap-2 flex-wrap">
            <el-button v-for="(seg, index) in createForm.segments" :key="index"
              :type="currentSegmentIndex === index ? 'primary' : 'default'" size="small" @click="selectSegment(index)"
              class="relative">
              <span class="flex items-center gap-1">
                <span class="w-3 h-3 rounded-full inline-block"
                  :style="{ backgroundColor: getSegmentColor(index) }"></span>
                {{ segmentLabel }} {{ index + 1 }}
              </span>
              <el-icon v-if="createForm.segments.length > 1" class="ml-1 hover:text-red-500 cursor-pointer"
            ...
            <div class="mt-2 rounded-lg border border-gray-200 bg-gray-50/40 px-4 py-3">
              <div class="flex items-center justify-between">
                ...
                <div class="text-xs text-gray-500">
                  {{ props.drawType === "Point" ? "点位" : "点位" }}：{{
                    props.drawType === "Point"
                      ? currentSegment.coords?.length === 2
                        ? 1
                        : 0
                      : currentSegment.coords?.length || 0
                  }}
                ...
                <el-button type="primary" plain @click="startSegmentDraw" :disabled="drawing">
                  {{
                    (
                      props.drawType === "Point"
                        ? currentSegment.coords?.length === 2
                        : currentSegment.coords?.length
                    )
                      ? "重新绘制"
                      : "开始绘制"
                  }}
                </el-button>
                <el-button @click="clearCurrentDraw" :disabled="drawing ||
                  !(props.drawType === 'Point'
                    ? currentSegment.coords?.length === 2
                    : currentSegment.coords?.length)
                  ">

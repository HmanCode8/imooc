...
addRequestInterceptor((config) => {
  ...
  // 如果 body 不是 FormData，则默认加 Content-Type
...
async function request(url, config = {}) {
  const { method = 'GET', headers = {}, body, timeout = TIMEOUT, withCredentials = true, params } = config
  ...
  try {
    // 发起请求 + 超时控制
    const response = await Promise.race([fetch(fullUrl, fetchConfig), timeoutPromise(finalConfig.timeout || TIMEOUT)])
    ...
    // 检查 HTTP 状态码
    if (!finalResponse.ok) {
      throw new RequestError(`HTTP ${finalResponse.status}: ${finalResponse.statusText}`, finalResponse.status, finalResponse.status)
    }
    ...
    if (data.code !== undefined && data.code !== 200) {
      throw new RequestError(data.message || '请求失败', finalResponse.status, data.code, data)
    }
    ...
    throw new RequestError(error instanceof Error ? error.message : '网络错误', 0, 0)
  }
}

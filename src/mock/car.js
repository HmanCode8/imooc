...
export const ENTERPRISE_OPTIONS = ['测试A', '测试B']
export const REGION_OPTIONS = ['禅城区', '南海区', '顺德区', '高明区', '三水区']
export const VEHICLE_TYPE_OPTIONS = ['正式', '测试']
export const VEHICLE_CATEGORY_OPTIONS = ['无人物流车', '无人货运车', '无人清扫车', '无人安防车']
...
const buildTerminalInfo = (car, index) => {
  ...
  const signalRealtime = base.signalRealtime ?? (offline ? '无' : index % 4 === 0 ? '左转' : index % 4 === 1 ? '右转' : index % 4 === 2 ? '双闪' : '正常')
  return {
...
/** 将原始车辆数据规范化为地图/列表可用结构 */
export function normalizeCar(car, index = 0) {
  const traj = pickTrajectory(car)
  const status = car.status === '离线' ? 'offline' : car.status
  const enterprise = car.enterprise ?? pickByIndex(ENTERPRISE_OPTIONS, index)
  const region = car.region ?? pickByIndex(REGION_OPTIONS, index)
  const type = car.type ?? pickByIndex(VEHICLE_TYPE_OPTIONS, index)
  const category = car.category ?? pickByIndex(VEHICLE_CATEGORY_OPTIONS, index)
  const terminalInfo = buildTerminalInfo({ ...car, status }, index)
  const actualRoute = car.actualRoute || traj.actualRoute || traj.plannedRoute || []
  const plannedRoute = car.plannedRoute || traj.plannedRoute || traj.actualRoute || []
  ...
  return {
    ...
    securityInfo: car.securityInfo
      ? {
          ...car.securityInfo,
          unit: car.securityInfo.unit ?? enterprise,
...
export const rawCarDataExport = rawCarData
export const carData = rawCarData.map((car, index) => normalizeCar(car, index))
